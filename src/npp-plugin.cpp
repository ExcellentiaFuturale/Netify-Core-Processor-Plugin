...
void nppChannelConfig::Load(const string &channel,
                            const json &jconf) {
  auto it = jconf.find("types");
  if (it != jconf.end() &&
      it->type() == json::value_t::array) {
  ...
  if (it != jconf.end() &&
    ...
    if (compressor == "none")
      ...
      throw ndPluginException("compressor",
                              strerror(EINVAL));
  }
}

nppPlugin::nppPlugin(const string &tag,
                     const ndPlugin::Params &params)
    : ndPluginProcessor(tag, params) {
  if (conf_filename.empty())
    throw ndPluginException("conf_filename",
                            strerror(EINVAL));
    ...
    if (jsinks != j.end()) {
      for (auto &jsink : jsinks->get<json::object_t>()) {
        for (auto &jchannel :
          ...
          config.Load(jchannel.first, jchannel.second,
                      defaults);
          ...
          if (sink == sinks.end()) {
            ...
            entry.insert(make_pair(jchannel.first, config));
...
void nppPlugin::DispatchPayload(
  ...
  for (auto &sink : sinks) {
    for (auto &channel : sink.second) {
      ...
      DispatchSinkPayload(sink.first, {channel.first},
                          jpayload, flags);
    }
  }
}
...
void nppPlugin::EncodeFlow(const nppFlowEvent &event,
  ...
  if (event.event == ndPluginProcessor::EVENT_FLOW_MAP) {
    ...
    if (it != jflows.end())
      ...
      jflows.insert(
          make_pair(event.flow->iface->ifname, jf));
    }
  }
  ...
  jpayload["internal"] =
      (event.flow->iface->role == ndIR_LAN);
  // XXX: Deprecated
  // jpayload["established"] = false;
  jpayload["flow"] = jflow;
}

void nppPlugin::EncodeAgentStatus(
    ndInstanceStatus *status) {
...
void nppPlugin::EncodeGlobalPacketStats(
    ndPacketStats *stats) {
  json jo;
  stats->Encode(jo);
...
void nppPlugin::DispatchLegacyPayload(void) {
  ...
  DispatchPayload(nppChannelConfig::TYPE_LEGACY_HTTP,
                  jpayload);
...
void nppPlugin::DispatchStreamPayload(void) {
  ...
  DispatchPayload(nppChannelConfig::TYPE_STREAM_STATS,
                  jpayload);
